...
const BOT_NAME_FANCY = 'SHALA MDᴮᴱᵀᴬ';
...
async function extractMessageBody(msg, sessionNumber) {
  ...
  try {
    switch (type) {
      ...
      case 'interactiveResponseMessage':
        try {
          const json = msg.message.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson;
          body = json ? JSON.parse(json)?.id : '';
        } catch {
          body = '';
        }
        break;
    ...
    // 🔢 Number system injection
    const ns = numberSystems.get(sessionNumber);
    if (ns?.handleNumberReply) {
      try {
        const injected = await ns.handleNumberReply(msg, body);
        if (typeof injected === 'string' && injected) body = injected;
      } catch (e) {
        console.error(`NumberSystem(${sessionNumber})`, e);
      }
    }

    return body.trim();
  } catch (err) {
    console.error('extractMessageBody error:', err);
    return '';
  }
}
// ---------------- command handler ----------------
...
function setupCommandHandlers(socket, number) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      const session = number.replace(/[^0-9]/g, '');
      const body = await extractMessageBody(msg, session);
      if (!body) return;
      ...
      for (const cmdData of commands) {
        if (cmdData.on === "body") {
          await cmdData.function(conn, mek, null, {
            from,
            body,
            isGroup,
            sender,
      ...
      const reply = (text, opt = {}) =>
        conn.sendMessage(from, { text, ...opt }, { quoted: msg });
...
async function setupMessageHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      const autoTyping =
        userConfig.AUTO_TYPING !== undefined
          ? userConfig.AUTO_TYPING
          : config.AUTO_TYPING;

      const autoRecording =
        userConfig.AUTO_RECORDING !== undefined
...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    // Initialize number system per socket/session
    const ns = initNumberSystem({
      conn: socket,
      mongoDB,
...
(async () => {
  try {
    await connectdb();  // mongoose settings connect
    await updb();       // sync settings to config
