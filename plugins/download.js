...
async (conn, mek, m, { from, prefix, q, reply }) => {
    ...
    const { footer, imageurl } = ownerdata;
        ...
        const fbResult = await fbDownloader(q);

        if (!fbResult || (!fbResult.hd && !fbResult.sd)) {
            return reply(N_FOUND);
        }

        const title = fbResult.title || "Facebook Video";
        const desc = fbResult.description || "No description";
        const thumb = fbResult.thumbnail || "";

        const hasHD = fbResult.hd ? true : false;
        const hasSD = fbResult.sd ? true : false;
        const hasAudio = hasSD || hasHD;
...
const ownerdata = (await axios.get(
    ...
    if (!fbResult || !fbResult.hd) {
	  ...
	  const thumbUrl = fbResult.thumbnail || "";
	  const title = fbResult.title || "Facebook Video";
...
resizedBotImg = await resizeImage(imgRes.data, 200, 200);
}
	  
    await conn.sendMessage(from, {
      document: { url: fbResult.hd },
      mimetype: "video/mp4",
      fileName: `${title}.mp4`,
      jpegThumbnail: resizedBotImg,
     caption: `\`720p (HD)\`\n\n${footer}`
...
cmd({
  ...
  try {
    ...
    if (!fbResult || (!fbResult.sd && !fbResult.hd)) {
      return reply("*📛 Audio not available*");
    }

    const audioUrl = fbResult.sd || fbResult.hd;
